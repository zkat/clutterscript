...
(function (factory) {
  ...
  set_reader_macro_function("'", function(stream) {
    return [intern("quote"), read(stream)];
  });
   ...
   * Compiler
   */
  var compile = exports.compile = function (x, env) {
    switch (typeof x) {
    case "object":
      return x.__clutter_compile(env);
    case "boolean":
      return x.toString();
    case "number":
      return x.toString();
    case "string":
      return x;
    default:
      throw new Error("Don't know how to compile this");
    }
  };

  Symbol.prototype.__clutter_compile = function(env) {
    return this.name;
  };

  Array.prototype.__clutter_compile = function() {
    return (compile(this[0]) + "("
            + this.slice(1).map(compile).join(", ")
            + ");");
  };
